 ...
 *=================================================================
 */
TIM_t* T_Start_millis (uint8_t timerType, uint16_t millis , uint32_t Prescaler)
{
	TIM_t* tim = get_timer(timerType);
...
void TDelay_Micros(uint16_t micros, uint32_t Prescaler)
{
	TDelay_Micro(T4 , micros,Prescaler);
}
/*
 *================================================================
 * @Fn				- TDelay_Millis
 ...
 *=================================================================
 */
void TDelay_Millis(uint16_t Millis, uint32_t Prescaler)
{
	TDelay_Milli(T4 , Millis, Prescaler);
}
...
void T_irq_micro_start(uint8_t timerType, uint16_t micros ,uint32_t Prescaler, void (* P_T_IRQ_CallBack)(void))
{
	TIM_t* tim = T_Start_micros(timerType ,micros, Prescaler );
	tim->DIER |= 1<<0;
...
void T_irq_millis_start(uint8_t timerType, uint16_t millis, uint32_t Prescaler, void (* P_T_IRQ_CallBack)(void))
{
	TIM_t* tim = T_Start_millis(timerType, millis, Prescaler );
	tim->DIER |= 1<<0;
	Enable_NVIC_TIM(timerType);
	GP_CallBack_TIM[timerType-1] = P_T_IRQ_CallBack;
}
